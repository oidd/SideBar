...
- `Sources/SideBar/AppDelegate.swift`
  App lifecycle, menu bar integration, settings window bootstrapping, permission entry points, and DockMinimize bridge lifecycle.
...
- `Sources/SideBar/AppConfig.swift`
  Persistent settings for enabled apps, colors, opacity, shortcuts, Dock exclusions, and shared app coordination.
- `Sources/SideBar/AppListManager.swift`
  Running app enumeration and filtering.
- `Sources/SideBar/SideBarBridge.swift`
  Distributed-notification and shared-file bridge for DockMinimize exclusions, hotkey runtime claims, mirrored bindings, and request/ACK routing.
